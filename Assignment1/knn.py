...
class KNN:
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i_test in range(num_test):
            ...
            dists[i_test] = np.sum(np.abs(np.repeat(X[i_test, np.newaxis], num_train, axis = 0)-self.train_X), axis=1)
        return dists
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            ind_nearest = np.argsort(dists[i])
            closest_y = self.train_y[ind_nearest[0:self.k]]
